...
if [ -z $1 ] ;
        then echo -e "\napticron sender's email address :"
        current_host=`cat /etc/yunohost/current_host`
        email_default=admin@$current_host
        echo -e "\n" ; read -e -p "(if you don't specify one, we will use $email_default) : " email_apti_s;
        if [ -z $email_apti_s ] ;
...
if [ -z $2 ] ;
        then echo -e "apticron reports receiving email address :"
	echo -e "\n" ; read -e -p "(if you don't specify an email, we will use $email_apti_s) : " email_apti_r;
	if [ -z $email_apti_r ] ;
...
if [ -z $3 ] ;
        then echo -e "fail2ban destination email address :"
	echo -e "\n" ; read -e -p "(if you don't specify one, we will use $email_apti_s) : " email_fail2ban;
	if [ -z $email_fail2ban ] ;
